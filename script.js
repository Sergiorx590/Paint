...
cnv.ontouchmove = (event) => {
    if(tool == "Кисточка"){
        ctx.fillStyle = color;
        ctx.fillRect(event.changedTouches[0].pageX- cnv.getBoundingClientRect().left - width/2, event.changedTouches[0].pageY - width/2, width, width);
    }
    else
        ctx.clearRect(event.changedTouches[0].pageX- cnv.getBoundingClientRect().left - width/2, event.changedTouches[0].pageY - width/2, width, width);
}
